...
with st.sidebar: 
    ...
    st.title("Automatic Machine Learning Application for *REGRESSION*")
...
if choice == "Welcome":
    ...
    st.error('Note that this application will prepare just REGRESSION models!!!', icon="🚨")

if choice == "Upload the Dataset":
    st.title("Upload Your Dataset")
    st.error('Note that this application will prepare REGRESSION models when uploading the dataset.', icon="⚠️")
    file = st.file_uploader("Upload Your Dataset for **REGRESSION**")
    if file: 
...
if choice == "Modelling ML Models":
    ...
    st.dataframe(df)
    st.error('Note that this application will prepare REGRESSION models when choosing the target feature.', icon="⚠️") 
    chosen_target = st.selectbox('Choose the Target Feature for the **REGRESSION** and click Run Modelling', df.columns)
    if st.button('Run Modelling'): 
        ...
        plot_model(best_model, plot = "rfe", display_format="streamlit")
